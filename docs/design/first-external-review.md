...
| Fleet network | `ec2:CreateNetworkAclEntry`, `ec2:ReplaceNetworkAclEntry`, `ec2:DeleteNetworkAclEntry`, `ec2:ReplaceNetworkAclAssociation`, `ec2:CreateRoute`, `ec2:ReplaceRoute`, `ec2:DeleteRoute`, `ec2:ReplaceRouteTableAssociation`, `ec2:DisassociateRouteTable`, `ec2:AuthorizeSecurityGroupEgress`, `ec2:RevokeSecurityGroupEgress`, `ec2:RevokeSecurityGroupIngress`, `ec2:ModifySecurityGroupRules`, `ec2:ModifyNetworkInterfaceAttribute`, `ec2:ModifyInstanceAttribute` (already present) | black-hole Loki egress for the fleet; the "no logs from a VM" alert cannot fire over a transport that is itself cut |
...
| Internet path (ahead of need) | `ec2:DetachInternetGateway`, `ec2:DeleteInternetGateway`, `ec2:AssociateRouteTable`, `ec2:DeleteNatGateway` | one-call fleet blackhole once subnets go private |
...
The Traps entry on "every route to the asset" gets the EBS direct API as its sixth found omission (the launch-path family in `iam.tf` was the fifth). `validation.tftest.hcl` gains one assertion per group (the rendered boundary contains the action). `docs/design/per-vm-aws-access.md` lists the additions in its boundary table.

The IGW/route-table one-call blackholes (`ec2:DetachInternetGateway`, `ec2:DeleteInternetGateway`, `ec2:AssociateRouteTable`, `ec2:DeleteNatGateway`) are dead under today's topology — subnets are public with `map_public_ip_on_launch`, so `DetachInternetGateway` fails on the mapped addresses, and each subnet has an explicit route-table association — but that cover is a property of the topology, not of the boundary, so the four are denied now rather than on the day the fleet moves behind a NAT gateway.
...
## 7. Live verification on the maintainer's staging fleet
...
2. **C2 path.** A second throwaway VM with a policy group that does not exist: after cloud-init finishes, `sudo -n true` as `ubuntu` fails, `/etc/sudoers.d/90-cloud-init-users` is absent, and `aws ec2 get-console-output --latest` carries the `BOOTSTRAP FAILED` line and the log.
...
Live, 2026-09-09, on the maintainer's own tenant re-pinned to `v1.1.0` / `base-1.1.0` (the first move off the private origin: plan 3 to add, 3 to change, 0 to destroy — boundary policy, instance and EIP tags, the log-shipping input guards; no instance replaced). Two throwaway `t3.large` VMs in eu-central-1, both `idlefy = "disabled"`.

- ChefSpec: 63 examples, 0 failures.
- **C1, before** (probe VM 1, staging group still on the previous cookbook): as `ubuntu`, `mv ~/.claude ~/.claude.bak; ln -s /etc/dev-vm ~/.claude`, then a root converge. `directory[/home/ubuntu/.claude]` and the two `create_if_missing` files ran through the symlink: `/etc/dev-vm` went `root:root 755` → `ubuntu:ubuntu 755`, `settings.json` and `statusline-command.sh` appeared inside it owned by `ubuntu`, and `touch /etc/dev-vm/probe` as `ubuntu` succeeded. `aws-access.env` itself stayed `root:root 600`; owning the directory is what makes it replaceable.
- **C1, after** (`make push` → staging on `base-1.1.0`, upgrade converge 2/181 resources updated in 22 s, every `ubuntu-*` copy skipped by its guard, no Traefik restart): same symlink, same converge. `execute[ubuntu-claude-statusline]` failed with `install: cannot create regular file '/home/ubuntu/.claude/statusline-command.sh': Permission denied`, the converge stopped there, `/etc/dev-vm` stayed `root:root 755` with its three files, and `touch` as `ubuntu` was refused. The developer can break their own converge; they can no longer reach root's directory.
- **C2** (probe VM 2, `policy_name = "no-such-policy"` — the module validates `policy_group`, so the name is the lever): cloud-init ended `status: error` 62 s after boot; as `ubuntu`, `sudo -n true` → "a password is required", `/etc/cinc/validation.pem` absent. `aws ec2 get-console-output --latest` fetched 30 s after the failure had no marker (the console buffer lags by up to about a minute); fetched again it carried `BOOTSTRAP FAILED: validation key removed; developer sudo stays revoked. Log follows.` and 49 lines ending in `FATAL: first converge failed; the converge log is /var/log/cinc-first-run.log on the instance and is not mirrored to the console`. Diagnosis of a bad policy is therefore the CINC server's node list, as §2 says.
- **Tailscale spelling** (§7.4, on probe VM 1 at `base-1.1.0`): `ln -s /usr/bin/sudo ~/"sudo tailscale"; ~/"sudo tailscale" login` from a uid-1000 SSH session. sudoers refused it (`user ubuntu is not allowed to execute '/usr/bin/login' as root`) and Falco emitted **nothing**: `proc.exepath` was `/usr/bin/sudo` and `proc.cmdline` was `sudo tailscale login`, the excluded spelling, because argv[0] is the symlink's name. The tripwire is quiet on a refused command, which is the case the exclusion was written to avoid. The permitted `sudo -n /usr/bin/tailscale login` in the same window fired as expected (`cmdline=sudo -n /usr/bin/tailscale login exepath=/usr/bin/sudo`). Fix: compare `proc.args` (argv without argv[0]) — `("tailscale login", "/usr/bin/tailscale login")` — instead of `proc.cmdline`; ships as `base-1.1.1`.
- **Falco limiter bucket** (§7.3): phase A, one `sudo -n id` with no flood → 1 detection in the journal. Phase B, 1300 `EACCES` opens of `/dev/shm/dev-vm-aws/x` in 3.0 s plus one `sudo -n id` at the end of the flood → 1301 lines under `falco-modern-bpf.service` in the journal (1300 credential-store, 1 sudo). Alloy's own counter after the run: `loki_process_dropped_lines_total{reason="ratelimit_drop_stage"} = 1098`, so about 200 of the 1301 reached `loki.write` — burst 200 plus 20/s, as configured. Whether the sudo line was among them is a Loki-side count (the tenant's token is `logs:write` only, by design — runbook §7 step 2 — so it is read from the stack's Explore, not from here), but the ruling does not wait for it: with 84 % of the window dropped, the detection's survival is chance, and `falco` joins the bypass selector in `base-1.1.1`. Windows (UTC): A 20:43:36–20:44:11, B 20:44:09–20:45:14, `{instance="<probe VM 1>", unit="falco-modern-bpf.service"} |= "Developer user invoked sudo"`.
- **`base-1.1.1` verified on the same VM before release** (rules file and Alloy selector dropped in place by hand): `falco -V` accepted the rules, the service restarted once with no restart loop, the symlink spelling now fires (`cmdline=sudo tailscale login exepath=/usr/bin/sudo`), and the same 1300-line flood plus one sudo gave 1301 journal lines with the limiter's drop counter unchanged — every Falco line reached `loki.write`.
- **Not a finding, worth knowing:** a wait loop that tried `ssh admin@` before the first converge had created that user tripped `fail2ban` (5 preauth failures in 600 s → 3600 s ban, `REJECT`), which reads as "connection refused" on port 22 while 80 and 443 still answer. Wait on `cloud-init status` as `ubuntu`, not on a user the cookbook creates.
